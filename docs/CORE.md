...
2. `jUtils.internal` - An object for some internal and undocumented APIs. Usage of them is bad practice. These properties may change in future versions!
...
4. `jUtils.init()` - Returns a new jUtils object. Don't use this to replace the current jUtils object. That's wasteful. Think about the old object's feelings.
5. `jUtils.getModules()` - Returns an array of the currently loaded modules. Modifying the returned array will not modify the original (internal) one.
6. `jUtils.remove("moduleName")` - Removes the module with the specified name. This is mainly for easy duct-tape fixes. If you want to remove a bundled module, please just consider building manually (recommended: sync up your copy of the jUtils repo to the official repo before each build).
...
3. Usage of a dev build in production and/or a production build in development
4. Doing manually what the wrapper functions already do. This will cause headaches and confusion later on.
